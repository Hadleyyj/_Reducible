...
def insert_word(s, hash_table):
    ...
    """
    size = len(hash_table)
    hashed= hash_word(s,size)
    step = step_size(s)
    while hash_table[hashed] != "":
...
def is_reducible(s, hash_table, hash_memo):
    ...
    for y in range(x):
        reduced = s[:y]+s[y+1:]
        if find_word(reduced, hash_table) and is_reducible(reduced, hash_table, hash_memo):
...
def main():
    ...
    # we do not know prior how many words will be reducible
    # let us assume it is 10 percent (fairly safe) of the words
    # then M is a prime number that is slightly greater than
    # 0.2 * size of word_list
    y= len(word_list)
    while is_prime(int(y*0.2)) is False:
        y+=1
    m = int(y*0.2)
